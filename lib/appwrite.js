...
export const getNotesAboutToExpire= async()=>{
    try {
        const notes= await database.listDocuments(appwriteconfig.databaseId,appwriteconfig.notesCollectionId,[Query.orderDesc('datelimit', Query.limit(7))])
...
export const getAllNotes= async()=>{
    try {
        const notes= await database.listDocuments(appwriteconfig.databaseId,appwriteconfig.notesCollectionId)
        return notes.documents
    } catch (error) {
        throw new Error(error);
