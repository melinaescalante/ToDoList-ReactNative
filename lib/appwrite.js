...
export const getNotesAboutToExpire = async (userId) => {
    try {
        const notes = await database.listDocuments(appwriteconfig.databaseId, appwriteconfig.notesCollectionId, [Query.equal('users', userId)], [Query.orderDesc('datelimit', Query.limit(7))])
...
export const getAllNotes = async (userId) => {
    try {
        console.log(userId, 'en notes')
        const notes = await database.listDocuments(appwriteconfig.databaseId, appwriteconfig.notesCollectionId, [Query.equal('users', userId
        )])
        return notes.documents
    } catch (error) {
        throw new Error(error);
...
export const createNote = async (note) => {
    try {
        const [thumnail, image] = await Promise.all([
            uploadFile(FormData.thumnail, 'thumbnail'),
            uploadFile(FormData.thumnail, 'image')
        ])
